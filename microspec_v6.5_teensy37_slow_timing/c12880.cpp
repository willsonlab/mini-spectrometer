...
void C12880_Class::read_into(String& datastr, uint16_t& peakvalue) {
  
  //compute integration time
   uint16_t  intensity;
   int pixeltarget=52;
  int duration_micros = (int) (_integ_time*1e3);
 ...
 datastr="";
  ...
  for(int i = 0; i < C12880_NUM_CHANNELS; i++){
    ...
    while(!_adc->isComplete()){}; //poll for completion  
     intensity= _adc->analogReadContinuous() ;
     datastr +=intensity;
   if  (i < C12880_NUM_CHANNELS) {
    datastr += ",";
    }
    else{
        datastr += "!";
 ...
 //     Serial.println(peakvalue);
  ...
  intensity=  _adc->analogRead(_VIDEO_pin);
